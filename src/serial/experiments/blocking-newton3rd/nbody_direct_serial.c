 ...
 *
 */
static void compute_accelerations_blocked_third_law (size_t              n,
                                                      dtype                g,
                                                      dtype                mass,
                                                      dtype                eps,
                                                      ...
                                                      )
{
  ...
  for (ib = 0u; ib < n; ib += BLOCK_SIZE)
    {
      const size_t  i_end = (ib + BLOCK_SIZE < n) ? ib + BLOCK_SIZE : n;

      for (jb = ib; jb < n; jb += BLOCK_SIZE)
        {
          const size_t  j_end = (jb + BLOCK_SIZE < n) ? jb + BLOCK_SIZE : n;
          ...
          for (i = ib; i < i_end; ++i)
            {
              ...
              dtype         ayi     = ay[i];
              dtype         azi     = az[i];
              const size_t  j_start = (jb == ib) ? i + 1u : jb;
...
#endif
      compute_accelerations_blocked_third_law (p->n, g, p->mass, eps,
                                   p->x, p->y, p->z,
                                   p->ax, p->ay, p->az);
#ifdef USE_PAPI
...
int main (int argc, char **argv)
{
  ...
  if (!quiet)
    {
      printf ("# serial direct N-body DKD blocking-newton3rd\n");
      ...
      if (profiler_path != NULL)
        {
          ...
          config.variant_name              = "blocking-newton3rd";
